...
resource "oci_database_db_system" "database" {
  ...
  hostname        = "${var.db_hostname_prefix}${substr(element(var.availability_domain, count.index), -1, 1)}"
...
locals {
  enable_dg = length(var.availability_domain) >= "2" && var.db_instance_count == 1 ? true : false
}
...
resource "oci_database_data_guard_association" "database_data_guard_association" {
  ...
  database_id                      = data.oci_database_databases.db[0].databases[0].id
  ...
  hostname            = "${var.db_hostname_prefix}dr${substr(element(var.availability_domain, count.index), -1, 1)}"
...
output "dbcon" {
  value = oci_database_db_system.database[*].db_home[*].database[*].connection_strings[*]["cdb_ip_default"]
}
